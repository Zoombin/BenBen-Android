...
public final class CaptureActivity extends Activity implements
		...
		public void handleMessage(Message msg) {
			...
			case PARSE_BARCODE_FAIL:
				...
				final InfoSimpleMsgHint hint = new InfoSimpleMsgHint(
						CaptureActivity.this, R.style.MyDialog1);
	...
	public void handleDecode(Result rawResult, Bitmap barcode) {
		...
		if (barcode == null) {
			...
			if (!url.startsWith(Link) && !url.startsWith(Link2)) {

				if (url.startsWith("http")) {
					inputDialog = new MsgDialog(CaptureActivity.this,
					...
					inputDialog = new MsgDialog(CaptureActivity.this,
							R.style.MyDialogStyle);
					inputDialog.setContent("检测到未知内容是否复制", url, "复制", "取消");
					inputDialog.setCancleListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							inputDialog.dismiss();
...
//			if (!isError) {
			String Content = "";
				if(url.contains(AndroidConfig.NETHOST)){
					Content = url.substring(54, url.length());
				}else{
					Content = url.substring(52, url.length());
				}
				...
				this.overridePendingTransition(R.anim.in_from_right,
						R.anim.out_to_left);
				intent.putExtra("id", urls[1]);
				startActivity(intent);
				finish();
			// } else {
			// final InfoSimpleMsgHint hint = new InfoSimpleMsgHint(
			// CaptureActivity.this, R.style.MyDialog1);
			// hint.setContent(errorInfo);
			// hint.show();
			// hint.setOKListener(new OnClickListener() {
			//
			// @Override
