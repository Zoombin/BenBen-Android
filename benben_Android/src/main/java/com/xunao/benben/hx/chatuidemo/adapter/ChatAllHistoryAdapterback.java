 ...
 * 
 */
public class ChatAllHistoryAdapterback extends ArrayAdapter<EMConversation> {
	...
	private HashMap<String, Object> huanXinMap;

	public ChatAllHistoryAdapterback(Context context, int textViewResourceId,
		...
		huanXinMap = CrashApplication.getInstance().getHuanXinMap();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// if(talkgroup.getHuanxin_groupid().equals(username)){
		// isGroup = true;
		// break;
		// }
		// }
		...
		if (isGroup) {
			// 群聊消息，显示群聊头像
			TalkGroup talkGroup = (TalkGroup) huanXinMap.get(username);

			holder.avatar.setImageResource(R.drawable.icon_default_grouppost);
			if (talkGroup != null) {
				holder.name.setText(talkGroup.getName());
			} else {
				holder.name.setText(contact.getUsername());
			}
		} else {

			Contacts contacts = (Contacts) huanXinMap.get(username);
			// 本地或者服务器获取用户详情，以用来显示头像和nick
			holder.avatar.setImageResource(R.drawable.icon_default_post);
			// TODO 以后添加群聊
			...
			holder.name
					.setText(contacts != null && contacts.getName() != null ? contacts
							.getName() : "未知");
		}
