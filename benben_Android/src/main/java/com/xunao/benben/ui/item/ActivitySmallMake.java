...
import com.xunao.benben.bean.SmallDataList;
...
public class ActivitySmallMake extends BaseActivity implements OnClickListener {
	...
	private ArrayList<SmallMakeData> mSmallMakeDataAll;
    ...
    class NewsBrocastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                if(creationAboutMeLists!=null && creationAboutMeLists.size()>0){
                    ll_new_message.setVisibility(View.VISIBLE);
                    tv_new_num.setText("您有"+creationAboutMeLists.size()+"条新消息");
                }else{
	...
	private void stopCurVoice() {
		if (VoicelistPlayClickListener.isPlaying
				&& VoicelistPlayClickListener.currentPlayListener != null) {
			// 停止语音播放
			VoicelistPlayClickListener.currentPlayListener.stopPlayVoice();
		}
	...
	public void loadLayout(Bundle savedInstanceState) {
        ...
        registerReceiver(brocastReceiver, new IntentFilter("hasCreationNews"));
	...
	public void initView(Bundle savedInstanceState) {
		setShowLoding(false);
		initTitleView();
		TitleMode mode = new TitleMode("#068cd9", "我的关注", 0,
				new OnClickListener() {
					@Override
	...
	private void refreshData(int type) {
		...
		case ALL:// 刷新全部
			isLoadMoreALL = false;
			InteNetUtils.getInstance(mContext).getSmallMakeData("",
					mRequestCallBack);
			break;
		}
	...
	private void loadMore(int type) {
		...
		case ALL:// 刷新全部
			isLoadMoreALL = true;
			InteNetUtils.getInstance(mContext).getSmallMakeData(
					mSmallMakeDataAll.get(mSmallMakeDataAll.size() - 1)
	...
	private void initlockData(int type) {
		...
		case MYSELF:
			try {
				...
				if (findAll != null && findAll.size() > 0) {
					...
					if (mSmallMakeDataAll.size() < AndroidConfig.DataNUM) {
		...
		case ALL:
			try {
				List<SmallMakeData> findAll = dbUtil.findAll(Selector
						.from(SmallMakeData.class));

				if (findAll != null && findAll.size() > 0) {
					mSmallMakeDataAll = (ArrayList<SmallMakeData>) findAll;
	...
	protected void onSuccess(JSONObject jsonObject) {
        spUtil.setLastCreationTime(new Date().getTime()/1000);
        CrashApplication.getInstance().creationNum=0;

		listview_all.onRefreshComplete();
		SmallDataList dataList = new SmallDataList();
		try {
			dataList.parseJSON(jsonObject);
			ArrayList<SmallMakeData> getmSmallMakeData = dataList
					.getmSmallMakeData();
	...
	private void saveData(ArrayList<SmallMakeData> getmSmallMakeData) {

		try {
			dbUtil.deleteAll(SmallMakeData.class);
			if (getmSmallMakeData != null && getmSmallMakeData.size() > 0) {
				dbUtil.saveOrUpdateAll(getmSmallMakeData);
				for (SmallMakeData s : getmSmallMakeData) {
	...
	private void addDataAll(ArrayList<SmallMakeData> getmSmallMakeData) {
		if (!isLoadMoreALL) {
			if (getmSmallMakeData != null && getmSmallMakeData.size() > 0) {
				mSmallMakeDataAll = getmSmallMakeData;
				all_nodota.setVisibility(View.GONE);
				if (mALLAdapter == null) {
					mALLAdapter = new MyAdapterAll(ALL);
				...
				if (mSmallMakeDataAll != null)
					mSmallMakeDataAll.clear();
				all_nodota.setVisibility(View.VISIBLE);
			}
		} else {
			if (getmSmallMakeData.size() < AndroidConfig.DataNUM) {
			...
			mALLAdapter.enterNum = true;
			mSmallMakeDataAll.addAll(getmSmallMakeData);
			mALLAdapter.notifyDataSetChanged();
...
//			}
			...
			InteNetUtils.getInstance(mContext).publicSmallComment(curentID, s,
					new RequestCallBack<String>() {
						...
						public void onSuccess(ResponseInfo<String> arg0) {
							...
							try {
								...
								try {
									...
									SmallMakeDataComment comment = new SmallMakeDataComment();
									comment.setReview(s);
									...
									switch (type) {
									case ALL:
										SmallMakeData smallMakeData = mSmallMakeDataAll
												.get(curentPosition);
										smallMakeData.getmFriendDataComments()
												.add(comment);
										mALLAdapter.notifyDataSetChanged();
										break;
									case MYSELF:
						...
						public void onFailure(HttpException arg0, String arg1) {
							// 评论失败
							dissLoding();
							ToastUtils.Errortoast(mContext, "发送失败,请重试");
						}
					});
		}
	}
	...
	private class MyAdapterAll extends BaseAdapter {
		...
		public int getCount() {
			// TODO Auto-generated method stub
			return mSmallMakeDataAll.size() + 1;
		}

		@Override
		public SmallMakeData getItem(int arg0) {
			// TODO Auto-generated method stub
			return mSmallMakeDataAll.get(arg0);
		}
		...
		public int getItemViewType(int position) {
			...
			if (position >= (type == ALL ? mSmallMakeDataAll.size()
		...
		public View getView(final int position, View contentView, ViewGroup arg2) {
			...
			if (contentView == null) {
				...
				case 0:
					...
					holderTextAImg.attention = (MyTextView) contentView
			...
			switch (itemViewType) {
			case 0:
				final SmallMakeData item = getItem(position);
				...
				if (getmFriendDataComments != null
					...
					for (final SmallMakeDataComment fc : getmFriendDataComments) {
				...
				if (item.getAttention() != 1) {
					holderTextAImg.attention.setVisibility(View.VISIBLE);
					holderTextAImg.attention
							.setOnClickListener(new OnClickListener() {

								@Override
								public void onClick(View v) {

									// if (Integer.parseInt(item.getMemberId())
									// ==
									// user
									// .getId()) {
																		...
																		public void onSuccess(
																			...
																			try {
																				...
																				try {
																					...
																					for (SmallMakeData s : mSmallMakeDataAll) {
																						if (s.getMemberId().equals(item
																								.getMemberId())) {
																							s.setAttention(1);
																						}
																					}
					...
					holderTextAImg.attention.setVisibility(View.GONE);
				}
...
//                        }
				...
				if (item.getType() == 0) {// 图文
					...
					if (images != null) {
						...
						holderTextAImg.item_friend_voice_box
								.setOnClickListener(new VoicelistPlayClickListener(
										item,
			...
			case 1:
				if (isMoreData) {
					...
					((TextView) contentView.findViewById(R.id.tv_loadmore))
							.setText("没有了,换个广场看看");
				}
				break;
			}
