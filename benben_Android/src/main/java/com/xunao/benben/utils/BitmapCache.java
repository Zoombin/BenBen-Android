...
import com.xunao.benben.ui.item.ActivityWriteFriend;
	...
	public void displayBmp(final ImageView iv, final String thumbPath,
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					if (isThumbPath && new File(thumbPath).exists()) {
				...
				if (thumb == null) {
//					thumb = ActivityWriteFriend.bimap;
                    thumb = MainActivity.bimap;
				}

				// if (isThumbPath) {
				// thumb = CommonUtils.readPictureDegree(thumbPath, thumb);
				// } else {
				thumb = CommonUtils.readPictureDegree(sourcePath, thumb);
				// }
