...
public class DragListView extends ListView {
    ...
    private boolean isLock = false;// 是否上锁.
    ...
    // x > dragger.getLeft() - 20这句话为了更好的触摸（-20可以省略）
                if (dragger != null && x > dragger.getLeft() - 20) {
                    ...
                    if(mDropListener!=null) {
                        mDropListener.start(temChangId);
                    }
        ...
        if (dragImageView != null && dragPosition != INVALID_POSITION
            ...
            switch (action) {
                case MotionEvent.ACTION_UP:
                    ...
                    onDrop(upY);
                    if(mDropListener!=null) {
                        mDropListener.stop(dragPosition);
                    }
                    break;
...
//windowParams.format = PixelFormat.TRANSLUCENT;// 默认为不透明，这里设成透明效果.
        ...
        convertView = LayoutInflater.from(getContext()).inflate(
                R.layout.item_list_move, null);
        ImageView imageView  =(ImageView) convertView.findViewById(R.id.iv_view);
...
//        ly.addView(imageView);
        ...
        windowManager.addView(convertView, windowParams);
    ...
    public void onDrag(int y) {
        ...
        if (dragImageView != null && drag_top >= 0) {
            ...
            windowParams.y = y - dragPoint + dragOffset;
            windowManager.updateViewLayout(convertView, windowParams);// 时时移动.
        }
...
// 数据交换
        if (dragPosition < getAdapter().getCount()) {
            if (dragPosition != temChangId) {
                if(mDropListener!=null) {
                    mDropListener.drop(temChangId, dragPosition);
                }
