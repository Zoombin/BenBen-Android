package com.xunao.test.base.IA;
...
public class DemoDuiTangImageReSizer extends DefaultImageResizer {
	...
	private static final int[] CDN_FIX_WIDTH_SIZE = { 110, 150, 170, 220, 240,
			290, 450, 580, 620, 790 };

	@Override
	public String getRemoteUrl(ImageTask imageTask) {
		String url = imageTask.getOriginUrl();
		int size = findBestCDNSize(CDN_FIX_WIDTH_SIZE,
				imageTask.getRequestSize().x, true);
		url = url.replace(TAG, TAG + DOT + size + SP + size);
		if (CubeDebug.DEBUG_IMAGE) {
			CLog.d("cube_image", "getRemoteUrl: %s %s",
					imageTask.getRequestSize(), url);
		}
		return url;
	}
